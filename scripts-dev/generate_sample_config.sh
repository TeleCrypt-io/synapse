...
if [ "$1" == "--check" ]; then
    if ! diff -u "$SAMPLE_CONFIG" <(synapse/_scripts/generate_config.py --header-file docs/.sample_config_header.yaml); then
        echo -e "\e[1m\e[31m$SAMPLE_CONFIG is not up-to-date. Regenerate it with \`scripts-dev/generate_sample_config.sh\`.\e[0m" >&2
        exit 1
    fi
    if ! diff -u "$SAMPLE_LOG_CONFIG" <(synapse/_scripts/generate_log_config.py); then
